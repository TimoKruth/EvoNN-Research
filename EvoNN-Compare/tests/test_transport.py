...
def test_transport_rejects_hash_mismatch_and_fully_rehashed_link(tmp_path,export_factory):
    ...
    with pytest.raises(ValueError,match='link or special'):
        hydrate_registry(registry,archive=malicious,descriptor={**descriptor,'archive_sha256':sha,'archive_size_bytes':size})
...
def test_archive_traversal_rejected_before_publication(tmp_path,export_factory):
    ...
    with pytest.raises(ValueError):
        hydrate_registry(registry,archive=bad,descriptor={**descriptor,'archive_sha256':sha,'archive_size_bytes':bad.stat().st_size})
...
def test_download_descriptor_is_validated_before_io_and_partial_is_retryable(tmp_path,monkeypatch):
    ...
    descriptor=dict(schema_version=1,archive_sha256='../../escaped',archive_size_bytes=10,index_sha256='0'*64,transport_manifest_sha256='0'*64,run_ids=['run'])
    declaration=dict(url='https://github.com/TimoKruth/EvoNN-Research/releases/download/test/evidence.tar.gz',descriptor=descriptor)
