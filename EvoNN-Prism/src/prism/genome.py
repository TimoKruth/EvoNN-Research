...
class ModelGenome(BaseModel):
    ...
    @property
    def genome_id(self):
        return canonical_sha256(self.model_dump(mode="json"), schema_version="prism.genome/v1", digest_field=None)
...
def mutate(genome, rng: Random, allowed, operator=None, task="classification"):
    ...
    if operator == "family" and len(allowed) > 1:
        ...
        updates["weight_decay"] = other(genome.weight_decay, [0, 0.001, 0.01, 0.1])
        ...
        updates["activation_sparsity"] = other(genome.activation_sparsity, [0.25, 0.5, 0.75])
