...
class CompiledPrimitive:
    ...
    def __init__(self, genome, input_shape, output_dim, modality, task, *, backend='numpy_fallback', device='cpu', seed=0):
        ...
        for index, primitive in enumerate(genome.primitives):
            ...
            if primitive.operator == 'sparse':
                ...
                self.masks[key] = ((np.arange(width)[:, None] - np.arange(width)[None, :]) % width < 2).astype(np.float32)
    ...
    def forward(self, parameters, features, *, training=False, seed=0):
        ...
        if self.token_input:
            ...
            if tokens.ndim != 2 or not np.isfinite(tokens).all() or np.any(tokens != np.floor(tokens)) or np.any(tokens < 0) or np.any(tokens >= self.output_dim):
